...
- **`YouTubeRequestFailedException`:** New exception with `statusCode` and
  `responseBody` fields for detailed HTTP error debugging.

## [1.0.5] - 2026-07-25

### Added

- **`VideoUnplayableException`:** New exception with `reason` and `subReasons` fields
  for detailed YouTube playability error reporting. Catches non-standard unplayable
  states with full error screen context.
- **`AgeRestrictedException`:** Specific exception for age-restricted videos
  (previously misclassified as `VideoUnavailableException`).

### Changed

- `_checkPlayabilityStatus()` now extracts YouTube error screen sub-reasons and
  throws more specific exceptions for each failure mode.
